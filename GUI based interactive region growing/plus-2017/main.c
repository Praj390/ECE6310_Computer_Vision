...
BOOL CALLBACK AboutDlgProc(HWND hDlg, UINT Message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		{
		case IDOK:
			predicate1 = GetDlgItemInt(hDlg, IDC_EDIT1, &val1, bSigned);
			if (!val1)
			{
				MessageBox(hDlg, "Please enter Predicate1", NULL, MB_OK);
				SendDlgItemMessage(hDlg, IDC_EDIT1, EM_SETSEL, 0, -1l);
			}
			predicate2 = GetDlgItemInt(hDlg, IDC_EDIT2, &val2, bSigned);
			if (!val2)
			{
				MessageBox(hDlg, "Please enter Predicate2", NULL, MB_OK);
...
switch (uMsg)
  ...
  case WM_COMMAND:
	  ...
	  case ID_COLOR_RED:
		  Red = (Red + 1) % 2;
		  if (Blue == 1 || Green == 1 || Cyan == 1 || Magenta==1 ||Yellow==1)
		  {
			  Green = Blue = Cyan = Yellow = Magenta = 0;
		  }
		  break;

	  case ID_COLOR_GREEN:
		  Green = (Green + 1) % 2;
		  if (Blue == 1 || Red == 1 || Cyan == 1 || Magenta == 1 || Yellow == 1)
		  {
			  Red = Blue = Cyan = Yellow = Magenta = 0;
		  }
		  break;

	  case ID_COLOR_BLUE:
		  Blue = (Blue + 1) % 2;
		  if (Green == 1 || Red == 1 || Cyan == 1 || Magenta == 1 || Yellow == 1)
		  {
			  Red = Green = Cyan = Yellow = Magenta = 0;
		  }
		  break;

	  case ID_COLOR_YELLOW:
		  Yellow = (Yellow + 1) % 2;
		  if (Blue == 1 || Red == 1 || Cyan == 1 || Magenta == 1 || Green == 1)
		  {
			  Red = Blue = Cyan = Green = Magenta = 0;
		  }
		  break;

	  case ID_COLOR_CYAN:
		  Cyan = (Cyan + 1) % 2;
		  if (Blue == 1 || Red == 1 || Green == 1 || Magenta == 1 || Yellow == 1)
		  {
			  Red = Blue = Green = Yellow = Magenta = 0;
		  }
		  break;

	  case ID_COLOR_MAGENTA:
		  Magenta = (Magenta + 1) % 2;
		  if (Blue == 1 || Red == 1 || Cyan == 1 || Green == 1 || Yellow == 1)
		  {
			  Red = Blue = Cyan = Yellow = Green = 0;
		  }
		  break;
...
if (ShowRegGrow == 1)
CheckMenuItem(hMenu, ID_DISPLAY_REGIONGROWING, MF_CHECKED);	/* you can also call EnableMenuItem() to grey(disable) an option */
else
...
if (Red == 1)
CheckMenuItem(hMenu, ID_COLOR_RED, MF_CHECKED);	/* you can also call EnableMenuItem() to grey(disable) an option */
else
CheckMenuItem(hMenu, ID_COLOR_RED, MF_UNCHECKED);

if (Green == 1)
CheckMenuItem(hMenu, ID_COLOR_GREEN, MF_CHECKED);	/* you can also call EnableMenuItem() to grey(disable) an option */
else
CheckMenuItem(hMenu, ID_COLOR_GREEN, MF_UNCHECKED);

if (Blue == 1)
CheckMenuItem(hMenu, ID_COLOR_BLUE, MF_CHECKED);	/* you can also call EnableMenuItem() to grey(disable) an option */
else
CheckMenuItem(hMenu, ID_COLOR_BLUE, MF_UNCHECKED);

if (Yellow == 1)
CheckMenuItem(hMenu, ID_COLOR_YELLOW, MF_CHECKED);	/* you can also call EnableMenuItem() to grey(disable) an option */
else
CheckMenuItem(hMenu, ID_COLOR_YELLOW, MF_UNCHECKED);

if (Cyan == 1)
CheckMenuItem(hMenu, ID_COLOR_CYAN, MF_CHECKED);	/* you can also call EnableMenuItem() to grey(disable) an option */
else
CheckMenuItem(hMenu, ID_COLOR_CYAN, MF_UNCHECKED);

if (Magenta == 1)
CheckMenuItem(hMenu, ID_COLOR_MAGENTA, MF_CHECKED);	/* you can also call EnableMenuItem() to grey(disable) an option */
else
CheckMenuItem(hMenu, ID_COLOR_MAGENTA, MF_UNCHECKED);
...
{
	int CenterRow, CenterCol,r, c, RegionSize, *indices, r2, c2;
	CenterRow = ThreadRow;
	CenterCol = ThreadCol;
	float avg, var;
...

{
	...
	int	average, distance, total;	/* average and total intensity in growing region */
	HDC	hDC;
	*count = 0;
		...
		if (Step_flag == 1) {
			...
			for (r2 = -1; r2 <= 1; r2++)
				for (c2 = -1; c2 <= 1; c2++)
				{
					...
					hDC = GetDC(MainWnd);
					if (Red == 0 && Yellow == 0 && Cyan == 0 && Green == 0 && Blue == 0 && Magenta ==0)
					{
					...
					if (Magenta == 1) {
						SetPixel(hDC, queue[qt] % COLS + c2, (queue[qt] / COLS + r2), RGB(255, 0, 255));
					}
