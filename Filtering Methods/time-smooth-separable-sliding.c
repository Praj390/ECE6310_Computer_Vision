	...
	** It smooths it using a separable 7x1 and 1x7 mean filter with sliding window.
...
for (c=0;c<COLS;c++)
  ...
  for (r=3;r<ROWS-3;r++)
  {
    for (r2=-3;r2<=3;r2++)
    {
      ...
      {
          sum = sum - image[(r+r2-1)*COLS+c] + image[(r+r2+6)*COLS+c];
          break;
      }
    }    
...
for (r=0;r<ROWS;r++)
  ...
  for (c=3;c<COLS-3;c++)
  {
    for (c2=-3;c2<=3;c2++)
    {
      if (c == 3)
      {
        sum+=smoothed_1[(c+c2)+r*COLS];
      }
      else
      {
          sum = sum - smoothed_1[(c+c2-1)+r*COLS] + smoothed_1[(c+c2+6)+r*COLS];
          break;
      }
    }
...
fpt=fopen("smoothed_1D_sliding_filter.ppm","wb");
